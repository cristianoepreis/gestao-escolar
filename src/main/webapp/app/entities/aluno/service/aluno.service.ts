...
type RestOf<T extends IAluno | NewAluno> = Omit<T, 'dataDeNascimento'> & {
...
export class AlunoService {
  ...
  protected convertDateFromClient<T extends IAluno | NewAluno | PartialUpdateAluno>(aluno: T): RestOf<T> {
    return {
      ...aluno,
      dataDeNascimento: aluno.dataDeNascimento?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restAluno: RestAluno): IAluno {
    return {
      ...restAluno,
      dataDeNascimento: restAluno.dataDeNascimento ? dayjs(restAluno.dataDeNascimento) : undefined,
    };
  }
